...
# 6) a) Create the variable command_override, and set it to be true or false. If command_override is false, then the K.shuttle should only launch if the fuel and engine check are O 
# If command_override is true, then the shuttle will launch regardless of the fuel and engine status.

    command_override = True

    if command_override:
     print("shuttle should only launch if the fuel and engine check are OK")
    else:
    print("huttle will launch regardless of the fuel and engine status.")


# If fuel_level is above 20000 AND engine_indicator_light is NOT red blinking OR 
# command_override is true print "Cleared to launch!" Else print "Launch scrubbed!"
